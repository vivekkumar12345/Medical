...
var datepickr = (function() {
	...
	handlers = {
		...
		documentClick: function(e) {
			if(e.target != this.element && e.target != this.Quicklook) {
				var parentNode = e.target.parentNode;
				if(parentNode != this.calender) {
					while(parentNode != this.Quicklook) {
	...
	function rebuildQuicklook() {
		while(this.QuicklookBody.hasChildNodes()){
			this.QuicklookBody.removeChild(this.QuicklookBody.lastChild);
		}
		...
		this.QuicklookBody.appendChild(buildDays(firstOfMonth, numDays, this.currentMonthView, this.currentYearView));
	}
	...
	function buildDays(firstOfMonth, numDays, currentMonthView, currentYearView) {
		var QuicklookBody = document.createDocumentFragment(),
		row = buildNode('tr'),
		dayCount = 0, i;
	...
	function buildQuicklook() {
		...
		var QuicklookContainer = buildNode('div', { className: 'Quicklook' });
		QuicklookContainer.style.cssText = 'display: none; position: absolute; top: ' + (inputTop + this.element.offsetHeight) + 'px; left: ' + inputLeft + 'px; z-index: 100;';
		...
		var Quicklook = buildNode('table', null, buildNode('thead', null, buildNode('tr', { className: 'weekdays' }, buildWeekdays())));
		this.QuicklookBody = buildNode('tbody');
		this.QuicklookBody.appendChild(buildDays(firstOfMonth, numDays, this.currentMonthView, this.currentYearView));
		Quicklook.appendChild(this.QuicklookBody);
		
		QuicklookContainer.appendChild(months);
		QuicklookContainer.appendChild(Quicklook);
		
		document.body.appendChild(QuicklookContainer);
		
		addEvent(QuicklookContainer, 'click', function(e) { handlers.QuicklookClick.call(self, e); });
	...
	return function(elementId, userConfig) {
		...
		this.open = function(e) {
			...
			self.Quicklook.style.display = 'block';
		}
		
		this.close = function() {
			removeEvent(document, 'click', self.documentClick);
			self.Quicklook.style.display = 'none';
		}
		
		this.Quicklook = buildQuicklook.call(this);
